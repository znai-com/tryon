...
.upload-area:hover { border-color: var(--accent); background: #f0f7ff; color: var(--accent); }
.compare-container { position:relative; width:100%; height:420px; background:#f4f4f4; border-radius:12px; overflow:hidden; margin: 15px 0; }
.compare-container img, #mask img { width:100% !important; height:100% !important; object-fit: contain !important; position:absolute; top:0; left:0; }
#mask { position:absolute; top:0; left:0; bottom:0; width:50%; overflow:hidden; border-right:2px solid #fff; z-index:5; }
.close-btn { position:absolute; top:12px; right:12px; width:28px; height:28px; background:#eee; border-radius:50%; display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:14px; transition: 0.2s; }
...
.main-trigger-btn { width:100%; margin-top:10px; padding:15px; background:#000; color:#fff; border-radius:10px; border:none; font-size:16px; font-weight:800; cursor:pointer; transition: transform 0.1s; }
...
<div class="tryon-box">
  ...
  <div id="step2" style="display:none; padding:20px 0;">
    ...
    <h3 style="font-size:16px;">AI Processing...</h3>
    ...
    try {
      const start = await fetch(BACKEND_URL+"/tryon/start", {
        ...
        body: JSON.stringify({
          ...
          storeUrl: window.location.hostname // 🔴 Sending hostname for verification
        })
      });

      const startData = await start.json();
      
      // 🔴 Check for Unauthorized or Expired Trial
      if (startData.disabled) {
          alert("Notice: " + startData.message);
          resetAll();
          return;
      }
      ...
      let finalUrl = null;
      for(let i=0; i<60; i++) { // Increased attempts for AI processing
        await new Promise(r=>setTimeout(r,3000));
        const statusRes = await fetch(BACKEND_URL+"/tryon/status/"+jobId);
        const s = await statusRes.json();
        
        if(s.status==="completed") { 
            finalUrl = s.resultUrl; 
            break; 
        }
        if(s.status==="failed") throw new Error("AI failed to process. Try a clearer photo.");
      }

      if(!finalUrl) throw new Error("Processing is taking too long. Please try again.");
